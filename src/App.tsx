...
const copy = {
  en: {
    signTitle: 'Employee Portal', signSub: 'Sign in with your King Bánh Mì Google account to reach your tools.', signBtn: 'Continue with Google', signOther: 'Use another account', signFoot: 'Only @kingbanhmi.com accounts have access.',
  ...
  vi: {
    signTitle: 'Cổng Nhân Viên', signSub: 'Đăng nhập bằng tài khoản Google King Bánh Mì để vào công cụ của bạn.', signBtn: 'Tiếp tục với Google', signOther: 'Dùng tài khoản khác', signFoot: 'Chỉ tài khoản @kingbanhmi.com được truy cập.',
...
function App() {
  ...
  return <div className={`app-frame ${expanded ? '' : 'sidebar-collapsed'}`}><Sidebar lang={lang} route={route} expanded={expanded} drawerOpen={drawerOpen} workspaceOpen={workspaceOpen} userName={userName} userEmail={user.email ?? 'frank@kingbanhmi.com'} onNavigate={go} onClose={() => setDrawerOpen(false)} onToggleExpanded={toggleExpanded} onToggleWorkspace={() => setWorkspaceOpen(old => !old)} onSignOut={logout}/><main className="app-main"><AppHeader lang={lang} route={route} name={userName} role={copy[lang].shiftRole} onMenu={() => setDrawerOpen(true)}/><div className="app-toolbar"><LanguageToggle lang={lang} onChange={setLang}/></div>{route === '/welcome' ? <Welcome lang={lang} firstName={firstName} onOpenChat={() => go('/sops-chat')}/> : <Chat key={lang} lang={lang}/>}</main></div>
}
